<?php
/**
 * Clase para interactuar con la API de iPos
 * @since 1.0.0
 */
...
class IPos_API {
    ...
    private $api_key;
    private $base_url;
    private $cache_key = 'ipos_sync_api_cache';
    private $cache_duration = 3 * HOUR_IN_SECONDS; // 3 horas
    
    public function __construct() {
        $this->api_key = get_option('ipos_sync_api_key');
        $this->base_url = get_option('ipos_sync_api_base_url');
    }
    ...
    /**
     * Petición genérica a la API de iPos
     */
    private function make_request($endpoint, $method = 'GET', $body = null) {
        ...
        $request_id = uniqid('ipos_sync_api_request_', true); // id único para petición
        $timeout = 420; // timeout de 7 minutos por petición
        ...
        do_action('ipos_sync_api_before_request', $endpoint, $method, $body, $request_id);
        ...
        $args = array(
            ...
            'timeout' => $timeout,
            'sslverify' => false // para desarrollo únicamente, remover en producción si es necesario
        );
        ...
        error_log('[iPos API] ✅ Respuesta serializada correctamente en ' . round(microtime(true) - $start_time, 2) . 's');
    ...
    /**
     * Obtener una categoría de producto específica
     */
    ...
    public function get_products($page = 0) {
        ...
        // cachear página si tiene productos
        if (!empty($products)) {
            set_transient($cache_key, $products, $this->cache_duration);
            error_log('[iPos API] 💾 Página ' . $page . ' cacheada');
        }
    ...
    public function get_all_products() {
        ...
        $cache_key = $this->cache_key . '_all_products';
        $cached_products = get_transient($cache_key);
        ...
        while ($has_more) {
            ...
            if (empty($page_products)) {
                ...
                // si la página tiene menos de 100 productos, es la última
                if ($page_count < 100) {
                    $has_more = false;
            ...
            // límite de seguridad
            if ($page > 500) {
                error_log('[iPos API] ⚠️ Límite de 500 páginas alcanzado');
                break;
            }
        }
        ...
        if (!empty($all_products)) {
            ...
            error_log('[iPos API] 💾 Todos los productos cacheados.');
        }
    ...
    /**
     * Limpiar toda la caché relacionada con iPos
     */
    public function clear_ipos_cache() {
        global $wpdb;

        // eliminar caché de productos
        delete_transient($this->cache_key . '_all_products');

        // eliminar todas las páginas de productos cacheadas
        $wpdb->query(
            $wpdb->prepare(
        ...
        // eliminar caché de categorías
        delete_transient($this->cache_key . '_categories');

        // eliminar cualquier otra caché relacionada con iPos
        do_action('ipos_sync_clear_cache', $this->cache_key);
        error_log('[iPos API] 🗑️ Caché de iPos eliminado');
        return true;
    }
